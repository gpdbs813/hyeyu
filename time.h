...
td,th{text-align:center;padding:10px;height:15px;width:100px}
    thead,tfoot{background:blue;color:whitesmoke}
    th{height:40px;width:400px}
    tbody tr:nth-child(even){background:#c0e1f5}
tbody tr:hover{background:#ffdfe4}
</style>
<script>
function showInfo(subject) {
    var info = document.getElementById("info");
    if (subject === "데이터사이언스기초") {
        info.innerHTML = "데이터 사이언스를 배우는 기초적인 과목 (신성교수님)";
    } else if (subject === "AI를 위한 기초수학") {
        info.innerHTML = "인공지능을 이해하는데 필요한 기초 수학 (신성교수님)";
    } else if (subject === "C프로그래밍") {
        info.innerHTML = "C언어를 이용하여 프로그래밍을 배우는 과목 (장현주교수님)";
    } else if (subject === "웹프로그래밍") {
        info.innerHTML = "웹 개발에 필요한 기초적인 프로그래밍 (장현주교수님)";
    }
